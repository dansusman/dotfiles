...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Prefer haiku for extraction, fall back to current model
			...
			if (haikuModel) {
				...
				if (auth.ok) extractionModel = haikuModel;
			}

			// Run extraction with loader UI
			const log = (msg: string) => appendFileSync("/tmp/answer-ext.log", `${new Date().toISOString()} ${msg}\n`);
			log("starting extraction");
			let extractionError: string | null = null;
			const extractionResult = await ctx.ui.custom<ExtractionResult | null>((tui, theme, _kb, done) => {
				log("inside ctx.ui.custom callback");
				const loader = new BorderedLoader(tui, theme, `Extracting questions using ${extractionModel.id}...`);
				loader.onAbort = () => { log("onAbort fired"); done(null); };
				log(`loader.signal.aborted = ${loader.signal.aborted}`);
				...
				const doExtract = async () => {
					...
					log(`calling complete() with model ${extractionModel.id} provider ${extractionModel.provider}`);
					const response = await complete(
						extractionModel,
						{ systemPrompt: SYSTEM_PROMPT, messages: [userMessage] },
						{ apiKey: auth.apiKey, headers: auth.headers, signal: loader.signal },
					);
					log(`response stopReason=${response.stopReason} content=${JSON.stringify(response.content).slice(0, 200)}`);
				...
				doExtract()
					.then((r) => { log(`extraction done: ${JSON.stringify(r)}`); done(r); })
					.catch((err) => {
			...
			if (extractionResult === null) {
				ctx.ui.notify(extractionError ? `Error: ${extractionError}` : "Cancelled", extractionError ? "error" : "info");
				return;
			}
